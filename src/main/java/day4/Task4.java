...
public class Task4 {
    public static void main(String[] args) {
        ...
        int maxSumThreeElementsIndex = 0;
        int maxSumThreeElements = 0;
        for (int i = 0; i < array.length - 2; i++) {
            int sumThree = 0;
            for (int j = i; j < i + 3; j++) {
                sumThree = sumThree + array[j];

                if (sumThree > maxSumThreeElements) {
                    maxSumThreeElements = sumThree;
                    maxSumThreeElementsIndex = i;
                }
            }
        }
        System.out.println(maxSumThreeElements);
        System.out.println(maxSumThreeElementsIndex);
    }
}